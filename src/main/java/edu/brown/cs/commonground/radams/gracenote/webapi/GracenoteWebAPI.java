package edu.brown.cs.commonground.radams.gracenote.webapi;
...
public class GracenoteWebAPI {
  ...
  public GracenoteWebAPI(String clientID, String clientTag, String userID)
      throws GracenoteException {
    // Sanity checks
    if (clientID.equals("")) {
  ...
  // should be stored
  // in a persistent form (filesystem, db, etc) otherwise you will hit your user
  // limit.
  ...
  public String register(String clientID) {
    ...
    if (!this._userID.equals("")) {
      System.out.println(
          "Warning: You already have a userID, no need to register another. Using current ID.");
      return this._userID;
    }
  ...
  // header, etc.
  protected String _constructQueryRequest(String body) {
  ...
  protected String _constructQueryRequest(String body, String command) {
    return "<QUERIES>" + "<AUTH>" + "<CLIENT>" + this._clientID + "-"
        + this._clientTag + "</CLIENT>" + "<USER>" + this._userID + "</USER>"
        + "</AUTH>" + "<QUERY CMD=\"" + command + "\">" + body + "</QUERY>"
        + "</QUERIES>";
  }
  ...
  protected String _constructQueryBody(String artist, String album,
    ...
    if (command.equals("ALBUM_FETCH")) {
      ...
      body += "<OPTION>" + "<PARAMETER>SELECT_DETAIL</PARAMETER>"
